 ...
 * Relies on app-level raw JSON body parser (req.rawBody) registered in app.ts.
 * Must NOT re-register application/json parser (FST_ERR_CTP_ALREADY_PRESENT).
 */
export async function wechatChannelRoutes(app: FastifyInstance): Promise<void> {
  app.post("/channels/wxpay/notify", async (req, reply) => {
