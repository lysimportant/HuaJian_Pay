...
| WeChat APIv3 key / merchant private key / platform public key | Critical | Env `WECHAT_APIV3_KEY`（或 `WECHAT_API_V3_KEY`）/ `WECHAT_PRIVATE_KEY` / Admin `api_v3_key`+`private_key`+`platform_public_key`; GET `/admin/api/channels/wxpay` never echoes secrets; PUT empty preserves; see `docs/wechat-pay.md` |
...
| Rotate `ADMIN_PASSWORD` from fresh-seed default `12345678`; ≥ 16 chars for production | **[Target]** ops（上线前必须修改） |
